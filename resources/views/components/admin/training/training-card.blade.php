                    ...
                    <x-common::form :method="'PATCH'" :action="route($updateTrainingRoute, ['id' => $training->id])" :buttonLabel="__('gym.save')">
                        <x-slot:content>
                            ...
                            <x-common::select :label="__('gym.instructor_name')" :name="'instructor_id'" :values="$instructorsMap" :current-value="$training->instructor->getFullName()"
                                :isDisabled="Auth::user()->role !== UserRole::ADMIN->value" @if (Auth::user()->role !== UserRole::ADMIN->value)
                                :needSend="false"
                                @endif :useValueId="true" />
                                <x-common::select :label="__('gym.training_template_type')" :name="'type'" :values="array_column(TrainingType::cases(), 'value')"
                                    :current-value="$training->type" :isDisabled="true" />
                                <x-common::textarea :label="__('gym.training_description')" :name="'description'" :value="$training->description" />
                                <x-common::input :label="__('gym.training_price')" :name="'price'" :isDisabled="true"
                                ...
                                <x-common::input :label="__('gym.training_max_clients')" :name="'max_clients'" :isDisabled="true"
                                    :value="$training->max_clients" />
                                <x-common::input :label="__('gym.training_start')" :name="'datetime_start'" :type="'datetime-local'"
                                    :value="$training->datetime_start" />
                                <x-common::input :label="__('gym.training_end')" :name="'datetime_end'" :type="'datetime-local'"
